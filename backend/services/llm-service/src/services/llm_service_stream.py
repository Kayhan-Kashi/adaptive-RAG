...
class LLMService:
    """Service for RAG-enhanced LLM operations with HyDE support and streaming"""
...
""")
    ...
    async def generate_stream(
        ...
        Yields:
            Tuple of (chunk_index, chunk_text, is_last)
        """
        if use_hyde is None:
        ...
        try:
            ...
            if not retrieved_chunks:
                ...
                error_msg = f"I don't have any relevant information in the selected documents to answer: '{prompt}'." if file_ids else "I don't have any relevant information to answer this question."
                yield (0, error_msg, True)
                return
            ...
            # ============ STEP 5: LLM Call 2 - Stream final answer ============
            logger.info("\n🤖 [STEP 4 - LLM Call 2] Streaming final answer...")
            ...
            async for chunk in self.llm.astream(formatted_messages):
                if chunk.content:
                    ...
                    yield (chunk_index, chunk.content, False)
                    chunk_index += 1
            
            # ============ ADD SOURCES ============
            # After streaming is complete, add sources if not already cited
            if "[Source:" not in full_answer and "[" not in full_answer:
                sources = []
                seen = set()
                ...
                if sources:
                    sources_text = "\n\n**Sources:**\n" + "\n".join(sources)
                    yield (chunk_index, sources_text, True)
                else:
                    # No sources to add, just mark as last
                    yield (chunk_index, "", True)
            else:
                # Already has citations, just mark as last
                yield (chunk_index, "", True)
            
            logger.info(f"\n💬 Response streamed: {len(full_answer)} characters")
            
        except Exception as e:
            logger.error(f"❌ Generation error: {e}")
            import traceback
            logger.error(traceback.format_exc())
            yield (0, f"Error generating response: {str(e)}", True)

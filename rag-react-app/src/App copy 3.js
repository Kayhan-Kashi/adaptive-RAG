...
function App() {
  ...
  const loadUploadedFiles = useCallback(async () => {
    if (!user) return;
    try {
      const response = await axios.get(`${API_BASE_URL}/upload/user/${user.id}`);
  ...
  const deleteFile = async (fileId) => {
    try {
      await axios.delete(`${API_BASE_URL}/upload/file/${fileId}`);
      await loadUploadedFiles();
            ...
            <div className="dashboard-content">
              ...
              <div className="upload-section">
                <label className="upload-btn">
                  Upload Documents
                  <input type="file" multiple accept=".pdf,.docx,.txt,.md" onChange={handleFileUpload} />
                </label>
